(ns marketentry.sim
  ...
  commit), then shows HARD-hold scenarios, including both branches of
  the Art.80/Art.81 preference-regime flagship check plus one clean
  preference claim."
...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (TCD, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up national-preference INELIGIBILITY branch) ==")
    ...
    (println "== filing/submit eng-4 (claims Art.80 national preference but fails the ownership/nationality test -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up dgi-record-unverified) ==")
    ...
    (println "== filing/submit eng-5 (dgi-record-unverified -> HARD hold) ==")
    (println (exec-op actor "t11" {:op :filing/submit :subject "eng-5"} operator))

    (println "== jurisdiction/assess eng-6 (Art.81 gender preference, ELIGIBLE and within 10% ceiling -> clean) ==")
    ...
    (println "== filing/submit eng-6 (clean Art.81 claim -- escalates for actuation, then commits) ==")
    (let [r (exec-op actor "t13" {:op :filing/submit :subject "eng-6"} operator)]
      (println r)
      (println (approve! actor "t13")))

    (println "== jurisdiction/assess eng-7 (sets up Art.128(1)(e) conflict of interest) ==")
    ...
    (println "== filing/submit eng-7 (procurement-official conflict of interest -> HARD hold) ==")
    (println (exec-op actor "t15" {:op :filing/submit :subject "eng-7"} operator))

    (println "== jurisdiction/assess eng-8 (sets up national-preference PRICE-CEILING branch) ==")
    ...
    (println "== filing/submit eng-8 (Art.80-eligible but claimed price exceeds the 15% ceiling -> HARD hold) ==")
