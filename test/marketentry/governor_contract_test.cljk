  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "N'Djamena Fabrication SARL"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "N'Djamena Fabrication SARL" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest national-preference-ineligibility-branch-is-held-and-unoverridable
  (testing "declared :national preference claim, fails BOTH eligibility branches (compliant price) -> HARD hold (flagship check, eligibility branch)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:preference-regime-ineligible} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest national-preference-price-ceiling-branch-is-held-and-unoverridable
  (testing "declared :national preference claim, IS eligible, but claimed price exceeds the 15% ceiling -> HARD hold (flagship check, price branch)"
    (let [[db actor] (fresh)
          _ (assess! actor "t5bpre" "eng-8")
          _ (draft! actor "t5bpre" "eng-8")
          res (exec-op actor "t5b" {:op :filing/submit :subject "eng-8"} operator)]
      (is (= :hold (get-in res [:state :disposition])))
      (is (some #{:preference-regime-ineligible} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest gender-preference-clean-claim-is-not-flagged-but-still-escalates-for-actuation
  (testing "a compliant :gender preference claim (eligible, within the 10% ceiling) is NOT a governor violation -- it still escalates because filing/submit is always high-stakes"
    (let [[db actor] (fresh)
          _ (assess! actor "t5cpre" "eng-6")
          _ (draft! actor "t5cpre" "eng-6")
          r1 (exec-op actor "t5c" {:op :filing/submit :subject "eng-6"} operator)]
      (is (= :interrupted (:status r1)) "pauses for human approval even when governor-clean")
      (let [r2 (approve! actor "t5c")]
        (is (= :commit (get-in r2 [:state :disposition])))
        (is (= 1 (count (store/submit-history db))))))))

(deftest procurement-official-conflict-of-interest-is-held-and-unoverridable
  (testing "Art.128(1)(e) conflict of interest declared true -> HARD hold"
    (let [[db actor] (fresh)
          _ (assess! actor "t5dpre" "eng-7")
          _ (draft! actor "t5dpre" "eng-7")
          res (exec-op actor "t5d" {:op :filing/submit :subject "eng-7"} operator)]
      (is (= :hold (get-in res [:state :disposition])))
      (is (some #{:procurement-official-conflict-of-interest} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
...
(deftest dgi-record-unverified-is-held-and-unoverridable
  (testing "unverified DGI tax record when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:dgi-record-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
